...
- **`flow it: <topic>`** / **`画流程图：<topic>`** *(flows module)* — turn a process into a Mermaid flowchart (start point, every step in order, every branch and where it goes, where it ends) plus a step-by-step what/why/next explanation, filed under `wiki/flows/`. Anything the source doesn't establish clearly gets flagged as Unclear, never guessed.
...
│   └── flows/           # optional: process/runtime flow diagrams (wiki/flows/) — pure schema, no scripts
│       └── CLAUDE.flows.md   # flow page format (Mermaid + step-by-step), "flow it" workflow
├── examples/
