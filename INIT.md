...
Ask as a **multi-select checklist**, split across two question groups since there are now six modules and the question tool caps at four options per question *(question tool if available: two questions in the same round — Group A covers the original four, Group B covers the other two; each option gets a one-line description of what gets installed; any subset across both groups can be picked)*:
...
All six are independent — any subset can be installed, including none. If a seventh module is ever added, split into three groups (or reconsider the grouping) — the tool's 4-option cap is the constraint, not the number "two."
