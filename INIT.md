...
Ask as a **multi-select checklist**, split across two question groups since there are now seven modules and the question tool caps at four options per question *(question tool if available: two questions in the same round — Group A covers the original four, Group B covers the other three; each option gets a one-line description of what gets installed; any subset across both groups can be picked)*:
...
- **related-wikis** — "Is this vault's subject matter connected to another vault built from this kit (or a compatible one) — e.g. a UI vault and its API vault, or a service and the platform vault it depends on?" If picked: installs `wiki/related-wikis.md` (a registry of sibling vaults and the boundary each owns), a cross-vault reference convention (plain-text `→ related: ...` — `[[wikilinks]]` don't resolve across vault folders), a **"link wiki"** trigger phrase to register or update an entry, a `link` log prefix, and a Lint check that registered paths still exist. Pure schema, no scripts. The related vault doesn't have to exist yet — an entry can point at a planned path and get promoted to a real one once that vault is actually initialized (same spirit as a red link).
- **tickets** — "Want a lightweight ticket/kanban system — `wiki/tickets/` (one file per ticket) plus a kanban board (`wiki/board.md`) with Backlog / In Development / In Review / Resolved columns?" If picked: installs the ticket page format, `wiki/board.md` (formatted for Obsidian's community "Kanban" plugin — a soft dependency: without the plugin the file still reads fine as plain markdown, just without drag-and-drop), **"create ticket"** / **"move ticket ... to ..."** trigger phrases, a `ticket` log prefix, and a Lint check that every ticket's frontmatter status matches the column its card sits under on the board. Pure schema, no scripts.
- **flows** — "Want end-to-end process/runtime flow diagrams — `wiki/flows/` (a Mermaid diagram + step-by-step explanation per process)?" If picked: installs the flow page format (start point, ordered steps, every decision branch and where it goes, where it ends; a per-step what/why/next explanation legible to both a non-technical reader and someone technical; anything the source doesn't establish clearly flagged as Unclear rather than guessed), the **"flow it" / "画流程图"** trigger phrase, a `flow` log prefix, and Lint checks for cross-linking and staleness. Pure schema, no scripts. Useful for any vault where "what steps does X actually go through, and what happens at each branch" comes up — a request flow, a build pipeline, a state machine, an approval process.

All seven are independent — any subset can be installed, including none. If an eighth module is ever added, split into three groups (or reconsider the grouping) — the tool's 4-option cap is the constraint, not the number "two."
...
7. *(related-wikis only)* **"Is there already another vault — or a planned one — that this wiki's subject matter connects to?"** — ask in chat; names, paths, and boundary descriptions are open-ended and a poor fit for option cards. For each one, get: the vault's name, its absolute path (or "not created yet — planned", with an expected path if known), and a one-line boundary of what each side owns (e.g. "this vault documents UI components and what they call; the API vault documents the endpoint contracts"). "None yet — I'll add this later with 'link wiki'" is a completely valid answer. → `{{RELATED_WIKIS}}` (may start empty).
8. *(tickets only)* **"What prefix do you want for ticket IDs?"** — ask in chat, open-ended (a short code, poor fit for option cards). Propose a short uppercase code derived from `{{PROJECT_NAME}}` as a default (or the generic `TICKET`), human confirms or names their own (e.g. a Jira-style project code they already use elsewhere). → `{{TICKET_PREFIX}}`.

**Done when:** every `{{PLACEHOLDER}}` referenced in Step 2 below has an answer (or an explicit "leave empty, fill in later"/"skip this" for #5, #6, and #7).
...
4. **Overwrite `README.md` with a short, vault-specific one** — a human landing on this repo (GitHub, a teammate's first look, browsing without Claude Code open) shouldn't have to open `CLAUDE.md` just to learn what the vault is and how to use it day-to-day. Replace the kit's own README content entirely — nothing about the kit's Quickstart, its own repo layout, or its `v0.1.0` status belongs here, since none of it describes the running vault:
   ...
   - **`link wiki`** *(related-wikis)* — register or update a sibling vault's entry in `wiki/related-wikis.md`; the related vault doesn't need to exist yet.
   - **`create ticket <title>`** *(tickets)* — file a new ticket in `wiki/tickets/`, add a card to the Backlog column of `wiki/board.md`.
   - **`move ticket <id> to <column>`** *(tickets)* — move it across Backlog/In Development/In Review/Resolved; updates both the card and the ticket's own frontmatter, no separate approval needed.
   - **`flow it: <topic>`** / **`画流程图：<topic>`** *(flows)* — turn a process into a Mermaid flowchart + step-by-step explanation (start point, every branch, where it ends), filed under `wiki/flows/`.
...
2c. *(related-wikis only)* For every entry in `wiki/related-wikis.md` with a real (non-`planned`) path, confirm the directory actually exists (`Test-Path <path>` / `test -d <path>`) — a typo'd path here would sit unnoticed until the next Lint run otherwise.
2d. *(tickets only)* Confirm `wiki/board.md` exists with all four column headers (`## Backlog`, `## In Development`, `## In Review`, `## Resolved`) and `kanban-plugin: board` in its frontmatter, and that `{{TICKET_PREFIX}}` was actually substituted (not left literal) anywhere it appears.
...
- **related-wikis installed** (appended): "`wiki/related-wikis.md` starts with whatever you named in the Interview — planned entries are fine to leave as-is until that vault actually exists. Say 'link wiki' any time to add another, or to promote a planned entry to a real path once that vault gets initialized."
- **tickets installed** (appended): "`wiki/board.md` starts empty — say 'create ticket' any time to add the first card. If you have Obsidian's community 'Kanban' plugin installed, open `board.md` there for a drag-and-drop view; without it, the file is still fully readable and editable as plain markdown."
- **flows installed** (appended): "`wiki/flows/` starts empty — say 'flow it: <topic>' any time you want a process turned into a diagram + step-by-step explanation. Obsidian renders Mermaid code blocks natively (no plugin needed) if you're browsing the vault there."
