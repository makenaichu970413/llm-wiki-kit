...
Ask as a **multi-select checklist**, split across two question groups since there are now five modules and the question tool caps at four options per question *(question tool if available: two questions in the same round — Group A covers the original four, Group B is just the fifth; each option gets a one-line description of what gets installed; any subset across both groups can be picked)*:
...
All five are independent — any subset can be installed, including none. If a sixth module is ever added, split into three groups (or reconsider the grouping) — the tool's 4-option cap is the constraint, not the number "two."
