...
> Append-only — entries are never deleted or rewritten. Prefix format: `## [YYYY-MM-DD] <op> | <subject>` where op ∈ init | ingest | query | answer | question | lint (installed modules add more: bootstrap | sync | config with code-sync, decision with decisions, plan with roadmap, link with related-wikis, ticket with tickets).
